...
class PlaceLimitBuyOrder():
	def __init__(self, kraken_client, kraken_BTC_balance, \
		...
		self.discount_to_average_bid = os.environ['kraken_discount_to_average_bid']
		self.minimum_balance = os.environ['kraken_BTC_minimum_balance']
		self.order_size = os.environ['kraken_BTC_order_size']
		...
		self.limit_bid_COIN = round(min(0.5*(self.high_bid_0_bittrex + self.high_bid_0_poloniex) \
			* (1 - self.discount_to_average_bid), self.high_bid_0_kraken + 0.00000001), os.environ['kraken_minimum_coin_order_size'])
	...
	def action_1(self):
		...
		try:
			buy_order = self.kraken.query_private('AddOrder',{'pair': os.environ['kraken_trading_pair'],'type':'buy','ordertype':'limit',\
...
class PlaceLimitSellOrder():
	def __init__(self, kraken_client, kraken_COIN_balance, \
		...
		self.premium_to_average_ask = os.environ['kraken_premium_to_average_ask']
